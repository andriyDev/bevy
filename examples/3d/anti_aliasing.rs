...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    // Plane
    let mesh = commands.spawn_asset(Mesh::from(Plane3d::default().mesh().size(20.0, 20.0)));
    let material = commands.spawn_asset(StandardMaterial::from(Color::srgb(0.1, 0.2, 0.1)));
    commands.spawn((Mesh3d(mesh), MeshMaterial3d(material)));

    let cube_texture = commands.spawn_asset(uv_debug_texture());
